// "use client";
// import Link from "next/link";
// import { useSession, signOut } from "next-auth/react";
// import { useState } from "react";
// import { usePathname } from "next/navigation"; // ✅ Add this
// import { FaAt, FaRegUser, FaTasks, FaRegPaperPlane } from "react-icons/fa";
// import {
//   MdOutlineAssuredWorkload,
//   MdOutlineCalculate,
//   MdOutlineCategory,
//   MdOutlineDashboard,
//   MdOutlineHandshake,
//   MdOutlineHeadphones,
//   MdOutlineLogout,
//   MdOutlineRealEstateAgent,
//   MdOutlineDiversity3,
//   MdOutlineContactPhone,
// } from "react-icons/md";
// import { SiConvertio } from "react-icons/si";
// import { RxCross2 } from "react-icons/rx";
// import { GiHamburgerMenu } from "react-icons/gi";
// import { IoSettingsOutline } from "react-icons/io5";
// import { t } from "@/components/translations";

// const MENU = {

//   admin: [
//     { href: "/dashboard", label: t("dashboard"), icon: <MdOutlineDashboard /> },
//     {
//       href: "/dashboard/category",
//       label: t("category"),
//       icon: <MdOutlineCategory />,
//     },
//     { href: "/dashboard/lead", label: t("lead"), icon: <MdOutlineContactPhone /> },
//     { href: "/dashboard/team", label: t("team"), icon: <MdOutlineDiversity3 /> },
//     { href: "/dashboard/calculator", label: t("calculator"), icon: <MdOutlineCalculate /> },
//     { href: "/dashboard/mailer", label: t("mailer"), icon: <FaRegPaperPlane /> },
//     // {
//     //   href: "/dashboard/setting",
//     //   label: t("setting"),
//     //   icon: <IoSettingsOutline />,
//     // },
//   ],
//   manager: [
//     { href: "/dashboard", label: t("dashboard"), icon: <MdOutlineDashboard /> },
//     { href: '/dashboard/lead', label: t("lead"), icon: <MdOutlineContactPhone /> },
//     { href: "/dashboard/calculator", label: t("calculator"), icon: <MdOutlineCalculate /> },
//     // { href: '/dashboard/manager/profile', label: t("profile"), icon: <FaRegUser /> },
//     { href: '/dashboard/manager/agenda', label: t("agenda"), icon: <FaTasks /> },
//     { href: "/dashboard/mailer", label: t("mailer"), icon: <FaRegPaperPlane /> },
//   ],
//   staff: [
//     { href: "/dashboard", label: t("dashboard"), icon: <MdOutlineDashboard /> },
//     { href: '/dashboard/lead', label: t("lead"), icon: <MdOutlineContactPhone /> },
//     { href: "/dashboard/calculator", label: t("calculator"), icon: <MdOutlineCalculate /> },
//     // { href: '/dashboard/manager/profile', label: t("profile"), icon: <FaRegUser /> },
//     { href: '/dashboard/manager/agenda', label: t("agenda"), icon: <FaTasks /> },
//     { href: "/dashboard/mailer", label: t("mailer"), icon: <FaRegPaperPlane /> },
//   ],
//   external: [
//     { href: "/dashboard", label: t("dashboard"), icon: <MdOutlineDashboard /> },
//     { href: '/dashboard/lead', label: t("lead"), icon: <MdOutlineContactPhone /> },
//   ],
//   user: [
//     { href: "/dashboard", label: t("dashboard"), icon: <MdOutlineDashboard /> },
//   ],

// };

// const ESTATE_MENU = {

//   admin: [
//     { href: "/dashboard", label: t("dashboard"), icon: <MdOutlineDashboard /> },
//     { href: "/dashboard/lead", label: t("lead"), icon: <MdOutlineContactPhone /> },
//     { href: "/dashboard/team", label: t("team"), icon: <MdOutlineDiversity3 /> },
//     { href: "/dashboard/calculator", label: t("calculator"), icon: <MdOutlineCalculate /> },
//     { href: "/dashboard/mailer", label: t("mailer"), icon: <FaRegPaperPlane /> },
//   ],
//   manager: [
//     { href: "/dashboard", label: t("dashboard"), icon: <MdOutlineDashboard /> },
//     { href: '/dashboard/lead', label: t("lead"), icon: <MdOutlineContactPhone /> },
//     { href: "/dashboard/calculator", label: t("calculator"), icon: <MdOutlineCalculate /> },
//     { href: '/dashboard/manager/agenda', label: t("agenda"), icon: <FaTasks /> },
//     { href: "/dashboard/mailer", label: t("mailer"), icon: <FaRegPaperPlane /> },
//   ],
//   staff: [
//     { href: "/dashboard", label: t("dashboard"), icon: <MdOutlineDashboard /> },
//     { href: '/dashboard/lead', label: t("lead"), icon: <MdOutlineContactPhone /> },
//     { href: "/dashboard/calculator", label: t("calculator"), icon: <MdOutlineCalculate /> },
//     { href: '/dashboard/manager/agenda', label: t("agenda"), icon: <FaTasks /> },
//     { href: "/dashboard/mailer", label: t("mailer"), icon: <FaRegPaperPlane /> },
//   ],
//   external: [
//     { href: "/dashboard", label: t("dashboard"), icon: <MdOutlineDashboard /> },
//     { href: '/dashboard/lead', label: t("lead"), icon: <MdOutlineContactPhone /> },
//   ],
//   user: [
//     { href: "/dashboard", label: t("dashboard"), icon: <MdOutlineDashboard /> },
//   ],

// };

// export default function Sidebar() {
//   const { data: session, status } = useSession();
//   const [open, setOpen] = useState(false);
//   const pathname = usePathname(); // ✅ current path

//   if (status === "loading") {
//     return (
//       <aside className="w-full shrink-0 border-r border-stock bg-white p-4">
//         <div className="text-muted-foreground">Loading menu...</div>
//       </aside>
//     );
//   }

//   if (!session) return null;

//   const access = session?.user?.access || [];
//   if (access.includes("estate")) {
//     let sidebar = ESTATE_MENU;
//   }else if (access.includes("mortgage")) {
//     let sidebar = MENU;
//   }else if (access.length === 0) {
//     let sidebar = MENU;
//   }else {
//     let sidebar = MENU;
//   }


//   const role = session?.user?.role || "user";

//   const items = MENU[role] || MENU.user;

//   return (
//     <>
//       {/* Mobile Toggle Button */}
//       <button
//         onClick={() => setOpen(true)}
//         className="p-2 md:hidden fixed top-4 left-4 z-50 bg-white rounded-lg shadow"
//       >
//         <GiHamburgerMenu size={24} />
//       </button>

//       {/* Sidebar */}
//       <aside
//         className={`flex flex-col bg-background h-full  shrink-0 border-r border-border  p-2 transform transition-transform duration-300
//         ${open ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}
//       >
//         {/* Close Button (Mobile Only) */}
//         <div className="flex justify-between items-center mb-4 md:hidden z-[999]">
//           <h2 className="font-semibold">Menú</h2>
//           <button onClick={() => setOpen(false)}>
//             <RxCross2 size={24} />
//           </button>
//         </div>

//         <div className="flex items-center gap-2 mb-4">
//           <FaAt size={20} className="text-[#F8C51B]" />
//           <h2 className="font-medium">{t("account")}</h2>
//         </div>

//         <nav className="flex-1 space-y-2  overflow-y-auto  pr-1 custom-scrollbar ">
//           <div>
//             <p className="text-sm text-muted-foreground">Mortgage Crm Menu</p>
//           </div>
//           {items.map((i, idx) => {
//             const isActive = pathname === i.href; // ✅ Check active

//             return (
//               <div key={idx}>
//                 <Link
//                   href={i.href}
//                   onClick={() => setOpen(false)}
//                   className={`flex items-center gap-2 rounded-lg px-3 py-2
//                     hover:bg-gray-100 transition-colors
//                     ${isActive ? "bg-[#21B57340]" : ""} `}
//                 >
//                   <span
//                     className={`${isActive ? "text-[#21B573]" : "text-[#84909A]"
//                       }`}
//                   >
//                     {i.icon}
//                   </span>
//                   <span
//                     className={`${isActive ? "text-[#21B573]" : "text-[#071437]"
//                       }`}
//                   >
//                     {i.label}
//                   </span>
//                 </Link>

//                 {i.label === "Agentes" && (
//                   <hr className="my-4 border-gray-300" />
//                 )}
//               </div>
//             );
//           })}
//           <div>
//             <p className="text-sm text-muted-foreground">Estate Crm Menu</p>
//           </div>
//         </nav>
//       </aside>
//     </>
//   );
// }


"use client";
import Link from "next/link";
import { useSession } from "next-auth/react";
import { useState } from "react";
import { usePathname } from "next/navigation";
import { FaAt, FaRegPaperPlane, FaTasks } from "react-icons/fa";
import {
  MdOutlineCalculate,
  MdOutlineCategory,
  MdOutlineDashboard,
  MdOutlineDiversity3,
  MdOutlineContactPhone,
} from "react-icons/md";
import { RxCross2 } from "react-icons/rx";
import { GiHamburgerMenu } from "react-icons/gi";
import { t } from "@/components/translations";

const MENU = {

  admin: [
    { href: "/dashboard", label: t("dashboard"), icon: <MdOutlineDashboard /> },
    {
      href: "/dashboard/category",
      label: t("category"),
      icon: <MdOutlineCategory />,
    },
    { href: "/dashboard/lead", label: t("lead"), icon: <MdOutlineContactPhone /> },
    { href: "/dashboard/team", label: t("team"), icon: <MdOutlineDiversity3 /> },
    { href: "/dashboard/calculator", label: t("calculator"), icon: <MdOutlineCalculate /> },
    { href: "/dashboard/mailer", label: t("mailer"), icon: <FaRegPaperPlane /> },
    // {
    //   href: "/dashboard/setting",
    //   label: t("setting"),
    //   icon: <IoSettingsOutline />,
    // },
  ],
  manager: [
    { href: "/dashboard", label: t("dashboard"), icon: <MdOutlineDashboard /> },
    { href: '/dashboard/lead', label: t("lead"), icon: <MdOutlineContactPhone /> },
    { href: "/dashboard/calculator", label: t("calculator"), icon: <MdOutlineCalculate /> },
    // { href: '/dashboard/manager/profile', label: t("profile"), icon: <FaRegUser /> },
    { href: '/dashboard/manager/agenda', label: t("agenda"), icon: <FaTasks /> },
    { href: "/dashboard/mailer", label: t("mailer"), icon: <FaRegPaperPlane /> },
  ],
  staff: [
    { href: "/dashboard", label: t("dashboard"), icon: <MdOutlineDashboard /> },
    { href: '/dashboard/lead', label: t("lead"), icon: <MdOutlineContactPhone /> },
    { href: "/dashboard/calculator", label: t("calculator"), icon: <MdOutlineCalculate /> },
    // { href: '/dashboard/manager/profile', label: t("profile"), icon: <FaRegUser /> },
    { href: '/dashboard/manager/agenda', label: t("agenda"), icon: <FaTasks /> },
    { href: "/dashboard/mailer", label: t("mailer"), icon: <FaRegPaperPlane /> },
  ],
  external: [
    { href: "/dashboard", label: t("dashboard"), icon: <MdOutlineDashboard /> },
    { href: '/dashboard/lead', label: t("lead"), icon: <MdOutlineContactPhone /> },
  ],
  user: [
    { href: "/dashboard", label: t("dashboard"), icon: <MdOutlineDashboard /> },
  ],

};

const ESTATE_MENU = {

  admin: [
    { href: "/estate/property", label: "Property List", icon: <MdOutlineDashboard /> },
    { href: "/estate", label: "Add Property", icon: <MdOutlineDashboard /> },
  ],
  manager: [
    { href: "/estate/property", label: "Property List", icon: <MdOutlineDashboard /> },
    { href: "/estate", label: "Add Property", icon: <MdOutlineDashboard /> },
  ],
  user: [
    { href: "/estate/property", label: "Property List", icon: <MdOutlineDashboard /> },
  ],

};

export default function Sidebar() {
  const { data: session, status } = useSession();
  const [open, setOpen] = useState(false);
  const pathname = usePathname();

  if (status === "loading") {
    return (
      <aside className="w-full shrink-0 border-r border-stock bg-white p-4">
        <div className="text-muted-foreground">Loading menu...</div>
      </aside>
    );
  }

  if (!session) return null;

  const access = session?.user?.access || [];
  const role = session?.user?.role || "user";

  // Logic to determine which menus to show
  const showMortgage = access.includes("mortgage") || access.length === 0;
  const showEstate = access.includes("estate");

  // Get the role-specific items for each menu
  const mortgageItems = MENU[role] || MENU.user;
  const estateItems = ESTATE_MENU[role] || ESTATE_MENU.user;

  return (
    <>
      {/* Mobile Toggle Button */}
      <button
        onClick={() => setOpen(true)}
        className="p-2 md:hidden fixed top-4 left-4 z-50 bg-white rounded-lg shadow"
      >
        <GiHamburgerMenu size={24} />
      </button>

      {/* Sidebar */}
      <aside
        className={`flex flex-col bg-background h-full shrink-0 border-r border-border p-2 transform transition-transform duration-300 
        ${open ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}
      >
        <div className="flex justify-between items-center mb-4 md:hidden z-[999]">
          <h2 className="font-semibold">Menú</h2>
          <button onClick={() => setOpen(false)}>
            <RxCross2 size={24} />
          </button>
        </div>

        <div className="flex items-center gap-2 mb-4">
          <FaAt size={20} className="text-[#F8C51B]" />
          <h2 className="font-medium">{t("account")}</h2>
        </div>

        <nav className="flex-1 space-y-4 overflow-y-auto pr-1 custom-scrollbar">

          {/* MORTGAGE SECTION */}
          {showMortgage && (
            <div className="space-y-2">
              <p className="text-xs font-bold uppercase text-muted-foreground px-3 mb-1">
                Mortgage CRM
              </p>
              {mortgageItems.map((i, idx) => (
                <SidebarLink key={`mortgage-${idx}`} item={i} pathname={pathname} setOpen={setOpen} />
              ))}
            </div>
          )}

          {/* Separator if both exist */}
          {showMortgage && showEstate && <hr className="border-border mx-2" />}

          {/* ESTATE SECTION */}
          {showEstate && (
            <div className="space-y-2">
              <p className="text-xs font-bold uppercase text-muted-foreground px-3 mb-1">
                Estate CRM
              </p>
              {estateItems.map((i, idx) => (
                <SidebarLink key={`estate-${idx}`} item={i} pathname={pathname} setOpen={setOpen} />
              ))}
            </div>
          )}
        </nav>
      </aside>
    </>
  );
}

/**
 * Extracted Link Component for cleaner code
 */
function SidebarLink({ item, pathname, setOpen }) {
  const isActive = pathname === item.href;
  return (
    <Link
      href={item.href}
      onClick={() => setOpen(false)}
      className={`flex items-center gap-2 rounded-lg px-3 py-2 hover:bg-gray-100 transition-colors ${isActive ? "bg-[#21B57340]" : ""
        }`}
    >
      <span className={isActive ? "text-[#21B573]" : "text-[#84909A]"}>
        {item.icon}
      </span>
      <span className={isActive ? "text-[#21B573]" : "text-[#071437]"}>
        {item.label}
      </span>
    </Link>
  );
}